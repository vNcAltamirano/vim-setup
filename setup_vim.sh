...
function! s:Fmt()
  ...
  elseif &filetype ==# 'nginx' && executable('nginxfmt')
    silent execute '%!nginxfmt -'
  elseif &filetype ==# 'yaml' && executable('yamlfmt')
    silent execute '%!yamlfmt -'
  else
...
iabbrev FFM ffmpeg -hide_banner -y -i input.mp4 -c:v libx264 -preset veryfast -crf 23 -c:a aac -b:a 128k output.mp4A


 
" =========================
" Configuración de ALE (Linter)
" =========================
let g:ale_linters = {
\   'python': ['ruff'],
\   'sh': ['shellcheck'],
...
" Mostrar errores en la columna de signos (la de la izquierda)
...
" Mapeos para saltar entre errores
